...
void GTSAMOptimization(std::vector<TLSPos> tlsVec, std::vector<CandidateInfo> candidates_vec, std::vector<OdomConstraint> odom_vec, gtsam::Values &result, std::pair<double, double> var)
{
    ...
    // add the odometry constraints between adjacent keyframes
    gtsam::noiseModel::Diagonal::shared_ptr odomNoise = gtsam::noiseModel::Diagonal::Variances((gtsam::Vector(6) << 1e-4, 1e-4, 1e-4, 1e-2, 1e-2, 1e-2).finished());
    for(int i=0; i<odom_vec.size(); i++)
    {
        gtsam::Point3 odomT(odom_vec[i].t);
        gtsam::Rot3 odomR(odom_vec[i].R);
        gtsam::Pose3 odometry(odomR, odomT);
        gtSAMGraph.add(gtsam::BetweenFactor<gtsam::Pose3>(odom_vec[i].fromID, odom_vec[i].toID, odometry, odomNoise));
    }
